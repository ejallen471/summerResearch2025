...
KDE-based skewness vary across x-grid indices.
...
1. Read replica data and calculate KDE and empirical skewness at each index.
2. Estimate uncertainty on both skewness calculations using bootstrap samples.
3. Plot skewness against grid index with error bars for every flavour.
"""
...
def calc_skewness(data, bandwidth, n_samplesMC):
    ...
    -------
    float
        Skewness of the KDE, where a symmetric distribution has
        skewness zero.
    """
...
def bootstrap_kde_skewness(data, bandwidth, n_bootstrap=250, n_samplesMC=20000):
    """Bootstrap standard error for KDE-based skewness.
    ...
    -------
    float
        Bootstrap standard error of the KDE-based skewness.
    """
    ...
    for i in range(n_bootstrap):
        ...
        resampled = data[idx]
        kurt_samples[i] = calc_skewness(resampled, bandwidth, n_samplesMC=n_samplesMC)

    return float(np.std(kurt_samples, ddof=1))


def bootstrap_empirical_skewness(data, n_bootstrap=250):
    """Bootstrap standard error for empirical skewness.
    ...
    -------
    float
        Bootstrap standard error of the empirical skewness.
    """
    ...
    for i in range(n_bootstrap):
        idx = np.random.randint(0, n, size=n)
        resampled = data[idx]
        kurt_samples[i] = calc_empirical_skewness(resampled)
...
def main():
    """Plot KDE-based and empirical skewness vs grid index.

    For each chosen flavour and each x-grid index, this function:

    1. Extracts the replica values for that (flavour, index) pair.
    2. Calculates the bandwidth via cross validation.
    3. Calculates the skewness empirically and via KDE.
    ...
    # Infer number of grid points from the first replica and flavour
    first_replica = res_flav[0]
    n_indices = len(first_replica[keys_flav[0]])
    indices = np.arange(n_indices)

    # Storage for results
    skewness_kde = {key: np.zeros(n_indices, dtype=float) for key in keys_flav}
    err_kde = {key: np.zeros(n_indices, dtype=float) for key in keys_flav}
    skewness_emp = {key: np.zeros(n_indices, dtype=float) for key in keys_flav}
    ...
    for key in keys_flav:
        ...
        for i, idx in enumerate(indices):
            ...
            # Bandwidth selection and KDE-based skewness
            bandwidth_matrix = calc_bandwidthMatrix(data)
            bandwidth = float(np.sqrt(bandwidth_matrix[0, 0]))

            k_kde = calc_skewness(data_1d, bandwidth, n_samplesMC=20000)
            k_kde_err = bootstrap_kde_skewness(
                data_1d,
                bandwidth,
                n_bootstrap=250,
                n_samplesMC=20000,
            ...
            # Empirical skewness and bootstrap error
            k_emp = calc_empirical_skewness(data_1d)
            k_emp_err = bootstrap_empirical_skewness(data_1d, n_bootstrap=250)

            skewness_kde[key][i] = k_kde
            err_kde[key][i] = k_kde_err
            skewness_emp[key][i] = k_emp
    ...
    axs[0].set_title("KDE-based skewness")
    axs[0].grid(True)

    # Right: Empirical skewness
    for key in keys_flav:
        axs[1].errorbar(
            indices,
            skewness_emp[key],
    ...
    axs[1].setTitle("Empirical skewness")
    axs[1].grid(True)
    ...
    plt.tight_layout()
    plt.savefig("skewness_vs_index_all_flavours.png", dpi=300)
    plt.show()
