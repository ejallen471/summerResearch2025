...
def calc_moments_importanceSampling_ALL(data, bandwidthMatrix, n_samplesMC):
    ...
    q_vals = multivariate_normal(mean=mu, cov=cov, allow_singular=True).pdf(samples)
    ...
    # First moment
    weighted_samples = weights[:, None] * samples  # shape (n_samplesMC, d)
    firstMomentVec = np.sum(weighted_samples, axis=0) / weight_sum

    # Second moment via einsum
...
# --- Calculate the bootstrap error for the empirical distributions
...
# --- run the 2d moment calculations with errors
...
# --- Assume no symmetry (basically doing all the loops)
        ...
        for idx2 in range(numberOfGridPoints):  
            for flav1 in keys_flav:
                for flav2 in keys_flav:
                    ...
                    # Update covariance and count matrices symmetrically
...
# --- plot the empirical and KDE constructions side by side
...
def main(plotting1D=False, empiricalReconstruction=True, integralReconstructionFull=True):
    ...
    keys_flav = ['d', 'u', 's', 'c']
    numberOfGridPoints = 8  # number of indices to include for integralReconstructionFull and empiricalReconstruction options
