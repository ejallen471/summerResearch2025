...
def calc_bootstrap_error_variance_skewness_1d(data, bandwidth, grid, differentialElement, n_bootstrap=100):

    n = len(data)
    zeorth_estimates = np.empty(n_bootstrap)
    mean_estimates = np.empty(n_bootstrap)
    var_estimates = np.empty(n_bootstrap)
    skewness_estimates = np.empty(n_bootstrap)
    ...
    for i in range(n_bootstrap):
        ...
        var = M2 - M1**2
        skewness = (M3 - 3*M1*M2 + 2*M1**3) / var**(3/2)

        zeorth_estimates[i] = M0
        mean_estimates[i] = M1
        var_estimates[i] = var
        skewness_estimates[i] = skewness

    # Compute the error
    zeorth_std = np.std(zeorth_estimates, ddof=1)
    mean_std = np.std(mean_estimates, ddof=1)
    var_std = np.std(var_estimates, ddof=1)
    skewness_std = np.std(skewness_estimates, ddof=1)

    return zeorth_std, mean_std, var_std, skewness_std
...
# --- Function to pass into calc_bootstrap_error_empirical that calculates the skewness
def skewness_stat(data_sample):
    empirical_mean = np.mean(data_sample)
    return np.mean((data_sample - empirical_mean) ** 3) / (np.std(data_sample, ddof=1) ** 3)
...
def run_1D_momentCalculations(data, bandwidthMatrix, n_bootstrap=50, n_gridIntegralResolution=1000):
    ...
    skewnessLst_integral = []
    skewnessErrorLst_integral = []
    skewnessLst_empirical = []
    skewnessErrorLst_empirical = []
    ...
    for dim in range(n_dims-1):
        ...
        # Calculate skewness (excess skewness)
        skewness = fourth_cumulant / (variance**2) - 3
        skewnessLst_integral.append(skewness)

        # Empirical moments
        empirical_mean = np.mean(data[:,dim])
        empirical_variance = np.var(data[:,dim], ddof=1)
        empirical_skewness = np.mean((data[:,dim] - empirical_mean) ** 4) / (np.std(data[:,dim], ddof=1) ** 4) - 3
        skewnessLst_empirical.append(empirical_skewness)
        ...
        bootstrapError_skewness = calc_bootstrap_error_empirical(data, skewness_stat, n_bootstrap)
        skewnessErrorLst_empirical.append(bootstrapError_skewness)

        # Integral bootstrap errors 
        bootstrapError_zeroth_is, bootstrapError_mean_is, bootstrapError_variance_is, bootstrapError_skewness_is = calc_bootstrap_error_variance_skewness_1d(data[:,dim], bandwidthMatrix, grid=grid, differentialElement=dx, n_bootstrap=100)
        skewnessErrorLst_integral.append(bootstrapError_skewness_is)
        ...
        print(f"skewness (KDE): {skewness} with error {bootstrapError_skewness_is}")
        print(f"skewness (Empirical): {empirical_skewness} with error {bootstrapError_skewness}\n")
    
    return skewnessLst_integral, skewnessErrorLst_integral, skewnessLst_empirical, skewnessErrorLst_empirical

#############################################################################
#############################################################################

# ---------------------------------------------
# --- Graphs of 450 dimensions stuff
# ---------------------------------------------

def filter_skewness_data(skewness_list, error_list, max_skewness=3):
    """Filters skewness values and corresponding errors based on a threshold."""
    filtered_skewness = []
    filtered_errors = []
    for k, err in zip(skewness_list, error_list):
        if k <= max_skewness:
            filtered_skewness.append(k)
            filtered_errors.append(err)
    return filtered_skewness, filtered_errors

def plot_skewness_histograms(integral_data, empirical_data):
    """Plots histograms of filtered skewness data."""
    fig, axs = plt.subplots(1, 2, figsize=(12, 5))

    axs[0].hist(integral_data, bins=100, color='skyblue', edgecolor='black')
    axs[0].set_xlabel('skewness')
    axs[0].set_ylabel('Frequency')
    axs[0].set_title('Histogram of skewness Values (Integral Method)', fontsize=12)

    axs[1].hist(empirical_data, bins=100, color='salmon', edgecolor='black')
    axs[1].set_xlabel('skewness')
    axs[1].set_ylabel('Frequency')
    axs[1].set_title('Histogram of skewness Values (Empirical Method)', fontsize=12)

    plt.tight_layout()
    plt.savefig("histogram_skewness.png", dpi=300)
    plt.show()

def save_skewness_results(integral_skewness, integral_errors, empirical_skewness, empirical_errors, filename="skewness_results.txt"):
    """Saves skewness data and errors to a text file."""
    with open(filename, 'w') as f:
        f.write(f'skewnessLst1_integral: {integral_skewness}\n')
        f.write(f'skewnessErrorLst1_integral: {integral_errors}\n')
        f.write(f'skewnessLst1_empirical: {empirical_skewness}\n')
        f.write(f'skewnessErrorLst1_empirical: {empirical_errors}\n')

def plot_skewness_with_errors(integral_data, integral_errors, empirical_data, empirical_errors):
    """Plots skewness values with error bars."""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6), sharey=True)

    ax1.errorbar(range(len(integral_data)), integral_data, yerr=integral_errors, fmt='o', capsize=5)
    ax1.axhline(0, linestyle='--', linewidth=1)
    ax1.set_title('Integral skewness')
    ax1.set_xlabel('Sample Index')
    ax1.set_ylabel('Integral skewness')
    ax1.grid(True)

    ax2.errorbar(range(len(empirical_data)), empirical_data, yerr=empirical_errors, fmt='s', capsize=5)
    ax2.axhline(0, linestyle='--', linewidth=1)
    ax2.set_title('Empirical skewness')
    ax2.set_xlabel('Sample Index')
    ax2.set_ylabel('Empirical skewness')
    ax2.grid(True)
...
def run_gaussian_kde_analysis(data, keys_test, n=100000, kdeGridRes=150, plotting1D=False, plotting2D=False):
    ...
    # --- BUILD BANDWIDTH MATRIX - we assume no covariance
    n, d = data.shape
    ...
    if plotting2D == True:
        ...
        x_dimension_idx = 0
        y_dimension_idx = 1  # or whichever two dims you want to plot
...
def main():
    ...
    # skewnessLst1_integral, skewnessErrorLst1_integral,  skewnessLst1_empirical, skewnessErrorLst1_empirical = run_1D_momentCalculations(data1, bandwidthMatrix)

    skewnessLst1_integralLst = []
    skewnessErrorLst1_integralLst = []
    skewnessLst1_empiricalLst = []
    skewnessErrorLst1_empiricalLst = []
    ...
    for selected_index in indices:
        ...
        # Collect results
        skewnessLst1_integralLst.extend(skewnessLst1_integral)  
        skewnessErrorLst1_integralLst.extend(skewnessErrorLst1_integral)
        skewnessLst1_empiricalLst.extend(skewnessLst1_empirical)
        skewnessErrorLst1_empiricalLst.extend(skewnessErrorLst1_empirical)

    # --- Run plots etc
    filtered_integral, filtered_integral_errors = filter_skewness_data(skewnessLst1_integralLst, skewnessErrorLst1_integralLst)
    filtered_empirical, filtered_empirical_errors = filter_skewness_data(skewnessLst1_empiricalLst, skewnessErrorLst1_empiricalLst)

    plot_skewness_histograms(filtered_integral, filtered_empirical)
    save_skewness_results(skewnessLst1_integralLst, skewnessErrorLst1_integralLst,skewnessLst1_empiricalLst, skewnessErrorLst1_empiricalLst)
    plot_skewness_with_errors(filtered_integral, filtered_integral_errors, filtered_empirical, filtered_empirical_errors)
